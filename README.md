# Hi, I'm Cameron 👋

📍 **Brighton, UK** | 🧭 **Senior Software Engineer @ Sentry** | 💻 **Indie Dev** | 🤖 **AI optimist** | 😎 **Vibe coding evangelist**

![Swift](https://img.shields.io/badge/-Swift-FA7343?style=flat-square&logo=swift&logoColor=white)
![SwiftUI](https://img.shields.io/badge/-SwiftUI-0D96F6?style=flat-square&logo=swift&logoColor=white)
![Xcode](https://img.shields.io/badge/-Xcode-1575F9?style=flat-square&logo=xcode&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Claude Code](https://img.shields.io/badge/-Claude%20Code-0A0A0A?style=flat-square&logo=anthropic&logoColor=white)
![Codex](https://img.shields.io/badge/-Codex-4B8DF8?style=flat-square&logo=openai&logoColor=white)
![AI Agents](https://img.shields.io/badge/-AI%20Agents-6C63FF?style=flat-square&logoColor=white)
![MCP](https://img.shields.io/badge/-Model%20Context%20Protocol-FF6B6B?style=flat-square&logo=protocol&logoColor=white)
![OSS](https://img.shields.io/badge/-Open%20Source-4B8DF8?style=flat-square&logo=github&logoColor=white)

## Philosophy

Just build. Have fun. AI has stripped away the barriers, making creation faster and cheaper than ever. Every idea you have is worth testing because the risk is almost nothing and the potential is unlimited. Don’t cling to failures; treat them as stepping stones. Right now is the greatest moment to be a developer. Forget perfection, chase outcomes.

## Current Projects

All my projects are open source and available on GitHub.

- **[XcodeBuildMCP](https://github.com/cameroncooke/XcodeBuildMCP)** – 2.9k+ ⭐ Highly popular MCP server for Xcode automation.
- **[AXe](https://github.com/cameroncooke/AXe)** – UI automation on Apple platforms made for AI agents and humans alike.
- **[reloaderoo](https://github.com/cameroncooke/reloaderoo)** – MCP development proxy & CLI MCP debug inspection tool.
- **[mcpli](https://github.com/cameroncooke/mcpli)** – Turns any MCP server into a first-class CLI.
- **[mcpusage](https://github.com/cameroncooke/MCPUsage)** - CLI for measuring MCP server tool advertisement token usage.

## GitHub Activity

![GitHub Contribution Graph](https://ghchart.rshah.org/cameroncooke)

## What I'm Doing

- **Building developer tools** – XcodeBuildMCP, AXe, mcpli, with a developer experience focus.
- **Writing at [Async Let](https://www.async-let.com/)** – Deep dives on Swift testing, MCP workflows, and balancing indie life.
- **Leading teams** – Coaching engineers while keeping hands-on with the latest Apple platform tech.
- **Experimenting with AI** – Trying to stay on the leading edge of AI and its impact on software development especially in relation to Apple platforms.

## Latest Blog Posts

<!-- BLOG-POST-LIST:START -->
- [Do MCP Servers Really Eat Half Your Context Window?](https://www.async-let.com/posts/claude-code-mcp-token-reporting/) _(November 19, 2025)_
- [Introducing MCPLI: Turn MCP servers into first class CLIs](https://www.async-let.com/posts/introducing-mcpli/) _(September 4, 2025)_
- [Finding Balance in the AI Boom: Family, FOMO, and My Limits](https://www.async-let.com/posts/family-fomo-and-ai/) _(August 10, 2025)_
- [My Take on the MCP vs CLI Debate](https://www.async-let.com/posts/my-take-on-the-mcp-verses-cli-debate/) _(July 5, 2025)_
- [The AI Coding Revolution: Magic or Mirage?](https://www.async-let.com/posts/agentic-reflections/) _(June 29, 2025)_
- [XcodeBuild MCP: Project Scaffolding, Swift Packages & Enhanced UI Automation](https://www.async-let.com/posts/xcodebuild-mcp-scaffolding-spm/) _(June 5, 2025)_
<!-- BLOG-POST-LIST:END -->

## Connect

[![GitHub](https://img.shields.io/badge/-Follow-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/cameroncooke)
[![X (Twitter)](https://img.shields.io/badge/-@camsoft2000-000000?style=flat-square&logo=x&logoColor=white)](https://x.com/camsoft2000)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Cameron%20Cooke-1C1C1C?style=flat-square&labelColor=0A66C2&color=1C1C1C)](https://www.linkedin.com/in/cameroncooke1/)
[![Blog](https://img.shields.io/badge/-Async%20Let-FF5722?style=flat-square&logo=astro&logoColor=white)](https://www.async-let.com)
